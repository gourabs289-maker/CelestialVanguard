...
// EMERGENCY DEFAULT CONFIG (In case Config.json fails)
let config = {
    general: { targetFPS: 60, scoreToTransition: 1000 },
    player: {
        space: { speed: 5, health: 100, fireRate: 150 },
        moon: { speed: 3.5, health: 150, fireRate: 250 }
    },
    enemies: {
        alien: { health: 30, speed: 2, score: 50, damage: 10 },
        robot: { health: 80, speed: 1, score: 150, damage: 25 }
    }
};

// IMAGE NAMES MATCHING YOUR SCREENSHOT
const images = {};
const assetPaths = {
...
async function init() {
    try {
        // Try loading your file, but don't crash if it fails
        const response = await fetch('Config.json');
        if (response.ok) {
            const data = await response.json();
            config = data;
        }
    } catch (e) { console.log("Using backup config"); }
...
function loadAssets() {
    for (let key in assetPaths) {
        ...
        img.onerror = () => { console.log("Failed to load: " + assetPaths[key]); };
        images[key] = img;
    }
}
...
class Player {
    ...
    update() {
        ...
        this.x += dx * stats.speed;
        this.y += dy * stats.speed;
    ...
    draw() {
        ...
        if (img && img.complete && img.naturalWidth !== 0) {
            ...
            // Draw a glowing triangle if image fails
            ctx.fillStyle = '#00f2ff';
            ctx.shadowBlur = 20; ctx.shadowColor = '#00f2ff';
            ctx.beginPath(); ctx.moveTo(0, -30); ctx.lineTo(25, 20); ctx.lineTo(-25, 20); ctx.closePath(); ctx.fill();
        }
        ctx.restore();
    }
}
...
class Enemy {
    ...
    draw() {
        ...
        const img = (this.type === 'alien') ? images.alien : null;
        if (img && img.complete && img.naturalWidth !== 0) {
...
function loop() {
    ...
    ctx.fillStyle = '#050505'; ctx.fillRect(0, 0, canvas.width, canvas.height); // Dark background
    
    const bg = (currentStage === 'NEBULA') ? images.nebula : images.moon;
    if (bg && bg.complete && bg.naturalWidth !== 0) {
        ctx.drawImage(bg, 0, 0, canvas.width, canvas.height);
    } else {
        // Starfield effect if background image fails
